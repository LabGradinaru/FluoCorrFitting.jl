@testset "extensions" begin
    ...
    @testset "shims (no optional deps loaded)" begin
        ...
        s = _errstr(() -> FCSFitting.fcs_plot(spec_for_plot, chp, [1.0, 0.0, 1e-3]))
        @test s !== nothing
        @test occursin("requires CairoMakie", s)

        s = _errstr(() -> FCSFitting._fcs_plot(spec_for_plot, chp, [1.0, 0.0, 1e-3]))
        @test s !== nothing
        @test occursin("requires CairoMakie", s)

        s = _errstr(() -> FCSFitting._resid_acf_plot(randn(10), 10))
        @test s !== nothing
        @test occursin("requires CairoMakie", s)

        s = _errstr(() -> FCSFitting.fcs_table(spec_for_plot, nothing, nothing))
        @test s !== nothing
        @test occursin("requires PrettyTables", s)

        s = _errstr(() -> FCSFitting.read_fcs("somefile.txt"))
        @test s !== nothing
    ...
    @testset "helpers (core logic)" begin
        ...
        colors = FCSFitting._fcs_colors_nt((c1, c2, c3))
        @test colors[:data] == c1
        @test colors[:fit]  == c2
        @test colors[:resid]== c3

        colors = FCSFitting._fcs_colors_nt((c1, c2))
        @test colors[:data] == c1
        @test colors[:fit]  == c2
        @test colors[:resid]== FCSFitting.DEFAULT_FCS_PLOT_COLORS[:resid]


        x = [1.0, 2.0, 4.0, 7.0]
        ρ = FCSFitting.acf(x; maxlag=2, demean=true, unbiased=true)
        ...
        # bounds / assertions
        @test_throws AssertionError FCSFitting.acf(randn(5); maxlag=0)
        @test_throws AssertionError FCSFitting.acf(randn(5); maxlag=5)

        Nbig = 5000
        xb = randn(Nbig)
        ρb = FCSFitting.acf(xb; maxlag=50)
        @test ρb[1] == 1
