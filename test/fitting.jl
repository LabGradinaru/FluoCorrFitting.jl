@testset "fitting" begin
    @testset "FCSFitResult constructor + StatsAPI shims" begin
        ...
        spec_free = FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none, n_diff=1, offset=nothing)

        scales = [2.0, 10.0, 0.5]
        ffr = FCSFitResult(lsf, spec_free, scales)

        lsf2 = FluorescenceCorrelationFitting._to_lfr(ffr)
        ...
        @test FluorescenceCorrelationFitting.mse(ffr) ≈ StatsAPI.rss(ffr) / StatsAPI.dof(ffr)
        @test FluorescenceCorrelationFitting.isconverged(ffr) == lsf.converged
    ...
    @testset "build_scales_from_p0 + build_scales" begin
        ...
        noscale = [5]
        θ0, s = FluorescenceCorrelationFitting.build_scales_from_p0(p0; noscale_idx=noscale, zero_sub=2.0)

        @test s ≈ [1.0, 2.0, 1e-3, 0.5, 1.0]
        @test θ0 ≈ [1.0, 0.0, 1.0, 1.0, 0.0]

        # wrapper does no protection → identical to noscale_idx=[]
        θ1, s1 = FluorescenceCorrelationFitting.build_scales(p0; zero_sub=2.0)
    ...
    @testset "infer_noscale_indices" begin
        ...
        spec = FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none, n_diff=1, offset=nothing)
        p0 = [1.0, 0.0, 1e-3, 1e-6, 0.2]
        idxs = FluorescenceCorrelationFitting.infer_noscale_indices(spec, p0)
        @test idxs == [5]
        ...
        spec = FCSModelSpec(; dim=FluorescenceCorrelationFitting.d3, anom=FluorescenceCorrelationFitting.perpop, n_diff=2, offset=0.0)
    ...
    @testset "fitting (2D Brownian, free offset)" begin
        ...
        model = FluorescenceCorrelationFitting.FCSModel(spec, τ, p0)
        y = model(τ, [g0_true, offset_true, τD_true])

        # Fit without bounds
        fit = FluorescenceCorrelationFitting.fcs_fit(spec, τ, y, p0)
        p̂ = coef(fit)
        ...
        lower = [1.0, -1e-1, 0.0]
        upper = [2.0, 1e-1, 1e-2]
        fit = FluorescenceCorrelationFitting.fcs_fit(spec, τ, y, p02; lower=lower, upper=upper)
        p̂ = coef(fit)
    ...
    @testset "fitting (3D Brownian, 2 diffusive components, with dynamics)" begin
        ...
        spec = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d3, anom=FluorescenceCorrelationFitting.none, n_diff=2, offset=0.0)
        ...
        lower = zeros(7)
        fit = FluorescenceCorrelationFitting.fcs_fit(spec, τ, y, p0; lower)
        p̂ = coef(fit)
    ...
    @testset "weights vs σ equivalence (heteroscedastic)" begin
        ...
        fitA = FluorescenceCorrelationFitting.fcs_fit(spec, ch, p0; upper)
        pA = coef(fitA)

        # Using explicit weights
        wt = @. 1 / σ^2
        fitB = FluorescenceCorrelationFitting.fcs_fit(spec, ch, p0; wt=wt, upper)
        pB = pA = coef(fitB)
    ...
    @testset "global fitting - shared τD (two channels, 2D Brownian)" begin
        ...
        spec = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none, n_diff=1)
        m    = FluorescenceCorrelationFitting.FCSModel(spec, τ, [0.5, 0.0, 1e-3])
        ...
        gfit = FluorescenceCorrelationFitting.fcs_fit(specs, [τ, τ], [y1, y2], p0s; shared=:τD)

        @test gfit isa FluorescenceCorrelationFitting.GlobalFCSFitResult
        @test FluorescenceCorrelationFitting.isconverged(gfit)

        # Shared τD should match truth
        τD_fit = FluorescenceCorrelationFitting.shared_coef(gfit)[1]
        @test τD_fit ≈ τD_true rtol=1e-4

        # Per-channel g0 and offset recovered via channel_coef
        p̂1 = FluorescenceCorrelationFitting.channel_coef(gfit, 1)
        p̂2 = FluorescenceCorrelationFitting.channel_coef(gfit, 2)
        ...
        r1 = FluorescenceCorrelationFitting.channel_result(gfit, 1)
        @test r1 isa FluorescenceCorrelationFitting.FCSFitResult
    ...
    @testset "global fitting - shared w₀ (two channels, fixed diffusivity)" begin
        ...
        spec1 = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none,
                                         offset=0.0, diffusivity=D1)
        spec2 = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none,
                                         offset=0.0, diffusivity=D2)

        m1 = FluorescenceCorrelationFitting.FCSModel(spec1, τ, [0.5, w0_true])
        m2 = FluorescenceCorrelationFitting.FCSModel(spec2, τ, [0.5, w0_true])
        ...
        gfit = FluorescenceCorrelationFitting.fcs_fit(specs, [τ, τ], [y1, y2], p0s;
                                   shared=:w0, lower=[0.0, 0.0, 0.0])

        @test FluorescenceCorrelationFitting.isconverged(gfit)

        w0_fit = FluorescenceCorrelationFitting.shared_coef(gfit)[1]
    ...
    @testset "global fitting – FCSChannel dispatch" begin
        ...
        gfit = FluorescenceCorrelationFitting.fcs_fit([spec, spec], [ch1, ch2],
                                   [[0.5, 0.0, 3e-4], [0.5, 0.0, 3e-4]]; shared=:τD)

        @test FluorescenceCorrelationFitting.isconverged(gfit)
        @test FluorescenceCorrelationFitting.shared_coef(gfit)[1] ≈ τD_true rtol=1e-3
    end

    @testset "global fitting – input validation" begin
        spec = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none)
        spec_fixedD = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none,
        ...
        @test_throws ArgumentError FluorescenceCorrelationFitting.fcs_fit([spec], [τ], [zeros(50)], [p0])
        # Unknown shared symbol
        @test_throws ArgumentError FluorescenceCorrelationFitting.fcs_fit([spec, spec], [τ, τ],
        ...
        spec_both = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none,
                                             offset=0.0, diffusivity=1e-11, width=3e-7)
        p0_both = [0.5]
        @test_throws ArgumentError FluorescenceCorrelationFitting.fcs_fit([spec_both, spec_both], [τ, τ],
                                                       [zeros(50), zeros(50)],
                                                       [p0_both, p0_both])
    end
    ...
    @testset "fixed diffusivity (w₀ fitted)" begin
        ...
        D = 5e-11
        w0_true = 5e-7 * rand()
        τD_true = FluorescenceCorrelationFitting.τD(D, w0_true)
        g0_true = rand()
        off_fixed = 0.0

        # Spec: 2D, Brownian, single diffuser, fixed offset & fixed D (so p = [g0, w0])
        spec = FluorescenceCorrelationFitting.FCSModelSpec(; dim=FluorescenceCorrelationFitting.d2, anom=FluorescenceCorrelationFitting.none, n_diff=1, offset=off_fixed, diffusivity=D)
        p0 = [0.001, 2e-9]
        model = FluorescenceCorrelationFitting.FCSModel(spec, τ, p0)
        ...
        fit = FluorescenceCorrelationFitting.fcs_fit(spec, τ, y, p0; lower=lower, upper=upper)
        p̂ = coef(fit)
        g0o, w0o = p̂

        @test g0o ≈ g0_true rtol=1e-4
        @test w0o ≈ w0_true rtol=1e-4
        @test FluorescenceCorrelationFitting.τD(D, w0o) ≈ τD_true rtol=1e-4
    end
end